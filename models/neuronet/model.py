...
#         self.frame_backbone = FrameBackBone(fs=self.fs, window=self.time_window, input_channel=2) # Nếu là đa kênh
...
class NeuroNet(nn.Module):
    def __init__(self, fs: int, second: int, time_window: int, time_step: float,
                 encoder_embed_dim: int, encoder_heads: int, encoder_depths: int,
                 decoder_embed_dim: int, decoder_heads: int, decoder_depths: int,
                 projection_hidden: List, input_channels: int = 1, temperature=0.01):
        super().__init__()
        self.fs = fs
        self.second = second
        self.time_window = time_window
        ...
        # 3. Projector cho Contrastive Learning
        project_layers = []
        curr_dim = encoder_embed_dim
        for h in projection_hidden:
            project_layers.append(nn.Linear(curr_dim, h))
            project_layers.append(nn.BatchNorm1d(h))
            project_layers.append(nn.ELU())
            curr_dim = h
...
class MaskedAutoEncoderViT(nn.Module):
    def __init__(self, input_size: int, num_patches: int,
        ...
        self.decoder_block = nn.ModuleList([
            Block(decoder_embed_dim, decoder_heads, 4., qkv_bias=True, norm_layer=nn.LayerNorm)
            for _ in range(decoder_depths)
        ])
