...
class FrameBackBone(nn.Module):
    def __init__(self, fs: int, window: int, input_channel: int = 1):
        super().__init__()
        self.model = BackBone(input_size=fs * window, input_channel=input_channel, layers=[1, 1, 1, 1])
        self.feature_num = self.model.get_final_length() // 2
    ...
    def forward(self, x):
        # x: (batch, n_frames, n_channels, frame_len)
        latent_seq = []
        for i in range(x.shape[1]):
            sample = x[:, i, :, :] # Lấy trọn n_channels
            latent = self.model(sample)
            latent_seq.append(latent)
...
class BackBone(nn.Module):
    ...
    def forward(self, x0):
        ...
        x1 = self.maxpool3(self.layer3x3_4(self.layer3x3_3(self.layer3x3_2(self.layer3x3_1(x0)))))
        x2 = self.maxpool5(self.layer5x5_4(self.layer5x5_3(self.layer5x5_2(self.layer5x5_1(x0)))))
        x3 = self.maxpool7(self.layer7x7_4(self.layer7x7_3(self.layer7x7_2(self.layer7x7_1(x0)))))
    ...
    def _make_layer3(self, block, planes, blocks, stride=2):
        ...
        self.inplanes3 = planes * block.expansion
        for _ in range(1, blocks): layers.append(block(self.inplanes3, planes))
        return nn.Sequential(*layers)
    ...
    def _make_layer5(self, block, planes, blocks, stride=2):
        ...
        self.inplanes5 = planes * block.expansion
        for _ in range(1, blocks): layers.append(block(self.inplanes5, planes))
        return nn.Sequential(*layers)
    ...
    def _make_layer7(self, block, planes, blocks, stride=2):
        ...
        self.inplanes7 = planes * block.expansion
        for _ in range(1, blocks): layers.append(block(self.inplanes7, planes))
        return nn.Sequential(*layers)
...
class BasicBlock3x3(nn.Module):
    ...
    def __init__(self, inplanes3, planes, stride=1, downsample=None):
        super().__init__()
        self.conv1 = nn.Conv1d(inplanes3, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn1 = nn.BatchNorm1d(planes)
        self.relu = nn.ELU(inplace=True)
...
class BasicBlock5x5(nn.Module):
    ...
    def __init__(self, inplanes5, planes, stride=1, downsample=None):
        super().__init__()
        self.conv1 = nn.Conv1d(inplanes5, planes, kernel_size=5, stride=stride, padding=2, bias=False)
        self.bn1 = nn.BatchNorm1d(planes)
        self.relu = nn.ELU(inplace=True)
    ...
    def forward(self, x):
        ...
        out = self.bn2(self.conv2(out))
        d = residual.shape[2] - out.shape[2]
        res_crop = residual[:, :, :-d] if d > 0 else residual
        return self.relu(out + res_crop)

class BasicBlock7x7(nn.Module):
    expansion = 1
    def __init__(self, inplanes7, planes, stride=1, downsample=None):
        super().__init__()
        self.conv1 = nn.Conv1d(inplanes7, planes, kernel_size=7, stride=stride, padding=3, bias=False)
        self.bn1 = nn.BatchNorm1d(planes)
        self.relu = nn.ELU(inplace=True)
